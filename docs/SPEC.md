...
No auth. Returns `{ "ok": true, "version": "<reference-impl-version>", "protocol": "drafts", "server_number": <n>, "runtime_capability": <bool>, ... }`. Used by load balancers and registry conformance checks. Reference implementations of v1.0+ that ship the `runtime` capability (§5.1) include `runtime_capability: true`.
...
### 3.5 Project bot runtime (v1.0+, PAP or SAP)

The `runtime` capability (§5.1) allows a project to ship server-executed code that handles Telegram bot updates and scheduled cron events. Reference implementation surface, honoured by beta.labs.vc and drafts.labs.vc:

```
GET    /drafts/project/bot/logs?limit=<n>&project=<n>
DELETE /drafts/project/bot/logs?project=<n>
```

`GET` returns `{ok, project, lines:[{at, level, line}], present, has_module, import_error, bot_js_mtime}`. `DELETE` clears the ring buffer. Both require PAP (auto-resolved to caller's project) or SAP (with explicit `?project=`).

Files inside the project's `live/` folder that drive runtime behaviour:

- `bot.js` — ESM module, `export default async function handler(update, ctx)` plus optional named exports for cron handlers
- `cron.json` — schedule list `[{schedule:"*/N * * * *"|"* * * * *", handler:"<name>"}]`

`ctx` surface available to user code: `ctx.kv` (per-project SQLite KV with `get`/`set`/`del`/`list`/`incr`, TTL, 10MiB/project + 1MiB/value caps), `ctx.send` (Telegram Bot API helper, token never reaches user code), `ctx.log(line)` (writes to ring buffer), `ctx.project`, `ctx.user_id`. Sandbox is Node `vm.createContext` with whitelisted globals; per-invocation 5-second wall-clock timeout. See `REFERENCE_IMPLEMENTATION.md` and CHANGELOG entry [1.0.0] for the full surface.

### 3.6 Reserved (not yet implemented)
...
|---|---|---|
| `static` | HTML, CSS, JS, fonts | shipped |
| `media` | Images, audio, video | shipped |
| `git` | Project has per-commit history and rollback | shipped |
| `github-sync` | Project mirrors to an external GitHub repo | shipped |
| `runtime` | Server-side code execution per project | shipped (reference impl v1.0+; protocol-level codification queued for 0.3) |
...
The reference implementation at v1.0 ships `runtime` ahead of the 0.3 protocol-level codification. The implementation surface for `runtime` is documented in §3.5, `REFERENCE_IMPLEMENTATION.md`, and CHANGELOG entry [1.0.0]. Other servers MAY ship `runtime` early following the same surface, OR MAY decline and continue advertising only `static, media, git, github-sync` until 0.3 lands.
...
### 7.10 Runtime sandbox

The v1.0+ reference implementation's `runtime` capability (§3.5, §5.1) executes user-supplied `bot.js` inside a Node `vm` context with a whitelisted globals set (no `fs`, `child_process`, `net`, `http`, `process`, `require`, dynamic `import`). Each invocation is bounded by a 5-second wall-clock `Promise.race` timeout. This is a **first-pass sandbox**, not a hard isolation primitive. It is sufficient against accidents and casual probes; it is NOT sufficient against a determined attacker. Implementations targeting untrusted operators SHOULD upgrade to `isolated-vm` or equivalent before exposing the runtime publicly. Hardening is queued for v1.1 of the reference implementation.
...
The protocol version (currently `0.2`) and the reference implementation version (currently `1.0.0`) are versioned independently. The reference implementation MAY ship features ahead of the protocol document; codification follows in subsequent protocol revisions.
