...
Canonical location: \`https://beta.labs.vc/drafts/registry.json\`
...
Server \`0\` is reserved for the reference server operated by Labs. Other non-negative integers assigned first-come via PR to the protocol repository's \`registry.json\`. Once assigned, a number MUST NOT be reassigned even if a server shuts down.
